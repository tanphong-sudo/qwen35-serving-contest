...
Root `docker-compose.yml` đã rollback byte-identical về best confirmed 65.06:
...
Adaptive completion cohort 20 đã score 49.12: ERC vẫn 1 và p95 tốt hơn nhẹ, nhưng TTFT p50
tăng 266 -> 371 ms và TBT tăng 26 -> 32 ms. Policy phát first token sớm rồi làm cohort mới
chờ decode, chuyển queue delay từ TTFT thành inter-token latency. Nhánh này đóng hoàn toàn;
không sweep window 16/24/32 hoặc biến thể defer-step khác.
...
| `CUSTOM-INFLIGHT-PREFIX` | `planned-research` | Rebase trực tiếp trên best 65.06; generic coalescing cho request đồng thời có shared token prefix: một prefix leader materialize KV/GDN state rồi followers reuse trước khi diverge | 20 request mỗi burst có common 6396-token prefix; prefix cache thông thường không loại toàn bộ duplicate work khi requests đến gần đồng thời. Sau thất bại scheduler, upside phải đến từ giảm tổng compute chứ không đổi nơi latency xuất hiện | TTFT p50 <=100–180 ms; p95 <300–600 ms; TBT không xấu hơn 26 ms | Chỉ build/nộp khi implementation generic, correctness test qua và projected score >=75 | Đây là lever chính tiếp theo; cấm hardcode prompt/hash/turn. Nếu không chứng minh được state reuse an toàn cho hybrid GDN/KV, dừng trước portal |
...
| 70–80 | TBT <=26 ms; TTFT p50 <180–250 ms, p95 <600–1000 ms | Giảm duplicate prefix/prefill compute; decode scheduler time-slicing đã bị loại |
...
- Thứ tự đã thực hiện: cap40 thất bại -> adaptive scheduler thất bại -> in-flight prefix research -> MTP/kernel finishing.
