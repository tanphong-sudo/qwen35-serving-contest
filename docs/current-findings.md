...
Root `docker-compose.yml` hiện là candidate adaptive scheduler đầu tiên, chờ metric portal:
...
Candidate giữ budget 2048 và toàn bộ flag của best 65.06, chỉ thêm custom
`CompletionCohortAsyncScheduler` với decode window 20. Build remote `linux/amd64`, import
smoke test, vLLM v0.24 API compatibility, compose validation và anonymous GHCR manifest đều
đã qua gate. Portal result là phép đo quyết định; không chỉnh thêm flag trước khi có metric.
...
| --- | --- | --- | --- | --- | --- | --- |
| `CUSTOM-ADAPTIVE-SCHED` | `ready-to-submit` | Snapshot `configs/vllm/submission-adaptive-cohort20.compose.yml`; compose SHA-256 `54e27c29e647bc5b826da907d45572b90bda9faac90c1c544ce218a21e99e29e`; public image `ghcr.io/tanphong-sudo/qwen35-adaptive@sha256:8a18315745a39d54085e1d99bfbb7e5ae55e5b6fb320132c7261abfa4dfc18db`; completion cohort window 20 | Default có TTFT 266/TBT 26; cap20 có TTFT 3499/TBT 18; cap40 có TTFT 292/TBT 26. Policy cho mọi prefill/first-token chạy, sau đó ưu tiên request ít output token còn lại nhất để cohort cũ hoàn thành trước | 29 tests pass; GitHub Actions run `29231204106` build `linux/amd64` và smoke-import thành công; anonymous manifest HTTP 200; target portal TBT 20–23 ms, TTFT p50 <250–400 ms, p95 <1000–1500 ms, ERC 1 | Nộp đúng root hiện tại, không ghép thêm flag. Promote nếu score >=66.06, failed 0, penalty 1; mở prefix phase nếu score >=70 và metric signature đạt | Nếu boot/runtime fail, ERC giảm, accuracy gate hỏng hoặc score <65.06: rollback byte-identical về snapshot 65.06. Nếu chỉ hòa/noise: đóng policy này và phân tích metric trước biến thể tiếp theo |
