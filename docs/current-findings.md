...
CUDA-graph exact cohort20 đã score 64.38: TTFT p50/p95 tốt hơn nhẹ 2/23 ms nhưng TBT xấu
26 -> 27 ms, kéo score xuống 0.68. Padding 20->24, 60->64, 100->104 không phải decode
bottleneck đáng kể; nhánh đóng hoàn toàn và không sweep thêm capture size/performance mode.

Scored snapshot thất bại:
...
| 50–70 | **Hiện tại 65.06:** TBT 26/TTFT 266; scheduler, fixed cap và CUDA graph retune đều đã đóng | Giữ rollback best; không còn compose-only candidate có evidence |
| 70–80 | TBT 23–25 ms; TTFT p50 <200–300 ms, p95 <900–1300 ms | Chỉ mở bằng local profiler/kernel/backend evidence; CUDA graph, prefix coalescing, MTP và decode time-slicing đã bị loại trước portal |
...
- Thứ tự đã thực hiện: cap40 thất bại -> adaptive thất bại -> prefix no-go -> CUDA graph thất bại -> MTP no-go -> profiler/kernel/backend research.
