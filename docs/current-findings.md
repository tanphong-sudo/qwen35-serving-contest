...
Root `docker-compose.yml` hiện là candidate vLLM v0.25.0 CUDA 12.9 version A/B:
...
Candidate giữ byte-identical best 65.06 ngoài đúng image digest. v0.25 chuyển dense models
sang Model Runner V2 và đã thêm Mamba/GDN align-prefix-cache path không cần per-step state
pre-copy. Upstream Qwen3.5 prefix-repetition benchmark gần workload cho TPOT 10.33 -> 9.68
ms (-6.29%) và TTFT gần như giữ nguyên. Áp tỷ lệ này vào TBT 26 ms cho proxy 24.36 ms,
tương ứng khoảng +5.19 điểm ở median-score proxy; ngay cả gain TBT 2% vẫn vượt promote gate.
...
| `V025-VERSION-UPGRADE` | `ready-to-submit` | Best 65.06 + chỉ đổi sang official v0.25.0 CUDA 12.9 amd64 digest `sha256:1a62fd4ad863259ec206e0d2b9fb24eb5d67b4deff87a1b2ae7889fc7f9ab23e`; snapshot `configs/vllm/submission-v025-upgrade.compose.yml`; compose SHA-256 `00a1deac0def9ce1be9e4a5e6bea7d7348582f0a7730e9aae04774c022460ac1` | Model Runner V2 trở thành default cho dense models; Qwen3.5 hybrid align cache path bỏ per-step state pre-copy và upstream benchmark gần workload cải thiện TPOT 6.29% với TTFT ngang | Conservative 2% TBT gain: 26 -> 25.48 ms, proxy +1.60; upstream ratio: 24.36 ms, proxy +5.19; reliability/accuracy kỳ vọng giữ nguyên vì model/config không đổi | Nộp đúng root, không ghép flag. Promote nếu score >=66.06, failed 0, penalty 1; strong win nếu TBT <=25 và ERC 1 | Boot/accuracy/score <65.06: rollback ngay. Nếu 65.06–66.05: ghi history nhưng không promote. Không retune v0.25 trước khi có metric |
...
| 50–70 | **Best 65.06:** TBT 26/TTFT 266; scheduler, fixed cap và CUDA graph retune đều đã đóng | v0.25 official version A/B là compose-only candidate duy nhất còn đủ evidence |
| 70–80 | TBT 23–25 ms; TTFT p50 <200–300 ms, p95 <900–1300 ms | v0.25 Model Runner V2 + hybrid align-prefix path; chỉ mở retune sau khi version A/B thắng |
...
- Thứ tự: cap40 thất bại -> adaptive thất bại -> prefix no-go -> CUDA graph thất bại -> MTP no-go -> v0.25 version A/B -> profiler/kernel/backend nếu cần.
...
v0.25 version A/B: failed = 0, penalty = 1, ERC = 1 và TBT <=25
  -> đúng strong-win signature; vẫn chỉ promote nếu final score >=66.06
...
- Official v0.25.0 CUDA 12.9 amd64 image digest là
  ...
  `dd10e03f95f94edbea1975c67ace3a35ec9a8a40`, compressed 11.74 GB; nhỏ hơn v0.24
  candidate hiện tại khoảng 3.3% và giữ đúng CUDA 12.9.1.
- Qwen3.5 MRV2 align-prefix benchmark trên H20, common prefix 4096 và concurrency 16 cho
  ...
  TTFT 174.91 -> 174.29 ms. Workload đủ gần để vượt gate version A/B nhưng không được coi là
  guarantee cho H200 MIG/trace thi.
