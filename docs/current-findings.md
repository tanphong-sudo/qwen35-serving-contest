...
Candidate v0.25.0 CUDA 12.9 đã boot-fail trước ready. Image khai báo CUDA 12.9.1 nhưng
`torchcodec` trong image load shared object liên kết `libnvrtc.so.13`; container chỉ có CUDA
12 nên thoát 1. Đây là packaging/runtime incompatibility, không phải tham số serving. Không
thử `LD_LIBRARY_PATH`, symlink, uninstall package hoặc đổi sang CUDA 13 trực tiếp trên portal
vì chưa có local H200 boot proof. Snapshot thất bại được giữ để không lặp lại:
...
| 50–70 | **Best 65.06:** TBT 26/TTFT 266; scheduler, fixed cap, CUDA graph và v0.25 image A/B đều đã đóng | Không có compose-only candidate an toàn; bước tiếp theo phải bắt đầu bằng profiler/local boot evidence |
| 70–80 | TBT 23–25 ms; TTFT p50 <200–300 ms, p95 <900–1300 ms | Runtime mới chỉ được xét sau khi chứng minh CUDA-consistent, boot sạch và giữ prefix-cache path |
...
- Thứ tự: cap40 thất bại -> adaptive thất bại -> prefix no-go -> CUDA graph thất bại -> MTP no-go -> v0.25 boot-fail -> profiler/local boot -> kernel/backend candidate.
