...
Root `docker-compose.yml` hiện là candidate CUDA-graph exact cohort20, chờ metric portal:
...
Candidate giữ byte-identical base 65.06 ngoài đúng một compilation config. Nó bảo toàn toàn
bộ 51 CUDA graph mặc định đến 512 và chỉ thêm ba size 20/60/100 còn thiếu. Vì vậy batch
20/60/100 không còn bị pad thành 24/64/104, còn mọi graph prefill/mixed lớn và hành vi
scheduler, prefix cache, quantization, model runtime đều giữ nguyên.
...
| `V024-CUDAGRAPH-COHORT20` | `ready-to-submit` | Best 65.06 + đúng một `cudagraph_capture_sizes` list giữ 51 default size đến 512 và thêm `20/60/100`; snapshot `configs/vllm/submission-v024-cudagraph-cohort20.compose.yml`; SHA-256 `455add01996d671ef26d895ba862a934b4e6841210df263bcc487621785e4a8b` | Default CUDA graph pad 20->24, 60->64, 100->104. Ba graph nhỏ bổ sung giữ exact burst-derived decode cohorts mà không bỏ coverage prefill/mixed mặc định | Bảo thủ 25% graph-sensitive: projected TBT 24.92 ms và median proxy +3.39 điểm; 50%: TBT 23.83 ms, proxy +6.96; TTFT/accuracy/ERC kỳ vọng giữ gần base | Nộp đúng root, không ghép flag. Promote nếu score >=66.06, failed 0, penalty 1; strong win nếu TBT <=25 và TTFT không xấu >100 ms | Nếu boot fail hoặc score <65.06: rollback base. Nếu TBT không đổi: CUDA padding không phải bottleneck, đóng branch; không thêm capture size khác |
| `MTP-1` | `research-conditional` | Base thắng sau CUDA-graph candidate + đúng một speculative token; không đổi scheduler cùng lượt | Nếu Qwen3.5/v0.24 có proposer tương thích, speculative decode có thể hạ TBT mà không đổi admission | Chỉ tạo candidate sau khi source xác nhận support, local boot/output contract pass và projected upside >=3; ưu tiên TBT <=22 ms, TTFT không xấu | Actual +1 để promote, failed 0, penalty 1 | Nếu model-native support thiếu, boot fail, throughput hoặc TTFT xấu: hủy toàn bộ MTP branch, không sweep số token/proposer |
...
| 70–80 | TBT 23–25 ms; TTFT p50 <200–300 ms, p95 <900–1300 ms | CUDA-graph exact cohort20 sizing; prefix coalescing và decode time-slicing đã bị loại |
| 80–90 | TBT 20–22 ms; TTFT p50 150–250 ms, p95 <600–1000 ms | MTP/speculative decode chỉ khi support được xác minh, hoặc A/B backend/runtime có local proof; không mở lại prefix coalescing khi chưa có profiler evidence |
...
- Thứ tự đã thực hiện: cap40 thất bại -> adaptive thất bại -> prefix no-go -> CUDA-graph cohort20 -> MTP/kernel finishing.
...
CUDA graph cohort20: TBT <=25, TTFT p50 không xấu quá 100 ms và ERC =1
  -> đúng metric signature; chỉ promote nếu final score vẫn >= best + 1
...
- Không nộp MTP trước khi xác minh model-native support, local boot và output contract.
